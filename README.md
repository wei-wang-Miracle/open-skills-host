...
> **让 AI 技能在任何地方运行** — 一个独立的 Skill 执行宿主，通过 HTTP API 让企业 AI 系统调用远程服务器上的结构化技能。
...
[![LangChain](https://img.shields.io/badge/powered%20by-LangChain-green.svg)](https://www.langchain.com/)
...
```mermaid
graph TB
    A[企业 AI 引擎 / 工作流平台] -->|POST /skills/invoke| B[open-skills-host]
    B --> C[技能发现]
    B --> D[ReAct Agent]
    C -->|扫描| E[SKILL.md 文件]
    D -->|调用| F[LLM]
...
graph LR
    ...
    subgraph "Skills API"
        A[GET /skills]
        B[GET /skills/name]
        C[POST /skills/invoke]
    end
    
    subgraph "响应数据"
        R1[SkillListResponse]
        R2[SkillDetail]
        R3[InvokeResponse]
    end
...
cd open-skills-host && uv sync

# 2️⃣ 配置环境变量
cp .env.example .env
# 编辑 .env，填入 OPENAI_API_KEY 和 SKILLS_DIR

# 3️⃣ 运行
uv run main.py --list                    # 查看可用技能
uv run server.py --port 8000             # 启动 HTTP 服务
```
...
请查阅 [CONTRIBUTING.md](CONTRIBUTING.md) 了解详细的贡献指南，包括如何设置开发环境、运行测试以及代码规范。

本项目遵循 [Contributor Covenant 行为准则](CODE_OF_CONDUCT.md)，欢迎友善、包容的社区贡献。
